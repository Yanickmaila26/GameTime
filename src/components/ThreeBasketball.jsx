...
export default function ThreeBasketball() {
  ...
  useEffect(() => {
    ...
    // 4. Lighting Configuration (Crepuscular Sunset Vibe)
    // Ambient: Deep dark navy/blue
    const ambientLight = new THREE.AmbientLight('#080e2b', 2.0);
    scene.add(ambientLight);

    // Directional 1: Warm glowing golden/orange (Sunset sun)
    const sunLight = new THREE.DirectionalLight('#FF6D00', 8.0);
    sunLight.position.set(-5, 3, 2);
    scene.add(sunLight);

    // Directional 2: Cold contrasting electric blue rim light
    const rimLight = new THREE.DirectionalLight('#00E5FF', 5.0);
    rimLight.position.set(5, -2, -2);
    scene.add(rimLight);

    // Point Light: Centered glow behind the ball
    ...
    const animate = (time) => {
      ...
      // Adjust lighting slightly based on scroll to dynamic reflections
